...
export default function PaymentDialog() {
    ...
    // We'll use this state to control the dialog opening/closing
    const [isDialogOpen, setIsDialogOpen] = React.useState(false)

    // Removed hasInstagram and hasTiktok as they aren't used in the payload or form.
    ...
    const savePaymentOptions = async (payload: any) => {
        ...
        if (user) {
            // --- FIX IS HERE: Changed .insert to .upsert with onConflict: 'user_id' ---
            const { error: profileError } = await supabaseClient.from('creator_profiles').upsert(
                {
                    ...
                    // Optionally, add a flag to mark payment details as set
                    has_payment_details: true,
                },
                {
                    onConflict: 'user_id', // This tells Supabase: if a profile with this user_id exists, UPDATE it instead of throwing an error.
                    ignoreDuplicates: false, // Keep this as false
                }
            )

            if (profileError) {
                console.error('Error saving profile:', profileError)
                toast.error(`Failed to save preferences: ${profileError.message}`) // Show the specific error message
            } else {
    ...
    const handleSubmit = async () => {
        const payload = {
            paymentMethod,
            accountNumber: paymentMethod === 'bank' ? accountNumber : null, // Use null instead of undefined for clarity in DB
    ...
    return (
        // Use the isDialogOpen state to control the Dialog
