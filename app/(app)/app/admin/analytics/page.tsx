...
interface Post {
    ...
    posted_at: string
    created_at: string
    creator_profiles: { full_name: string } | null
}
...
function engRate(p: PostWithInsight): string {
    ...
    return (((p.insight.likes + p.insight.comments) / p.insight.views) * 100).toFixed(2) + '%'
}
...
function avgEngRate(posts: PostWithInsight[]): string {
    ...
    const avg = published.reduce((acc, p) => {
        return acc + ((p.insight!.likes + p.insight!.comments) / p.insight!.views) * 100
    }, 0) / published.length
    return avg.toFixed(2) + '%'
}

function downloadCSV(posts: PostWithInsight[], campaignName: string): void {
    const headers = ['Media ID', 'Platform', 'Status', 'Posted At', 'Views', 'Likes', 'Comments', 'Engagement Rate', 'Last Updated']
    const rows = posts.map((p) => [
        p.media_id, p.platform, p.status,
        new Date(p.posted_at).toLocaleDateString(),
        p.insight?.views ?? 0, p.insight?.likes ?? 0, p.insight?.comments ?? 0,
        engRate(p),
...
const CustomPieTooltip = ({ active, payload }: any) => {
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-xl shadow-lg px-4 py-3 text-sm">
            ...
            <p className="text-black">{fmt(payload[0].value)} ({payload[0].payload.pct}%)</p>
        </div>
    )
}

// ── Sub-components ────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, accent, icon }: { label: string; value: number; sub?: string; accent: string; icon: string }) {
    return (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-shadow duration-200 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-24 h-24 rounded-full opacity-5 -translate-y-8 translate-x-8 transition-all duration-300 group-hover:opacity-10 group-hover:scale-110" style={{ background: accent }} />
            <div className="flex items-start justify-between mb-3">
                <span className="text-xs font-semibold tracking-widest uppercase text-gray-400">{label}</span>
                <span className="text-lg">{icon}</span>
            </div>
            <div className="text-3xl font-bold text-gray-900 tracking-tight mb-1" style={{ fontVariantNumeric: 'tabular-nums' }}>
                {fmt(value)}
            </div>
            {sub && <div className="text-xs text-gray-400 font-medium">{sub}</div>}
            <div className="absolute bottom-0 left-0 w-full h-0.5 opacity-60" style={{ background: `linear-gradient(to right, ${accent}, transparent)` }} />
        </div>
    )
}
...
function StatusPill({ status }: { status: string }) {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${map[status] ?? 'bg-gray-100 text-gray-600'}`}>
            <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${status === 'PUBLISHED' ? 'bg-emerald-500' : status === 'PROCESSING' ? 'bg-amber-500' : 'bg-red-500'}`} />
            {status}
        </span>
    )
}
...
function CampaignStatusPill({ status }: { status: string }) {
    ...
    return (
        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold capitalize ${map[status] ?? 'bg-gray-100 text-gray-500 ring-1 ring-gray-200'}`}>
            {status}
        </span>
    )
}
...
function SortTh({ label, k, cur, dir, onSort }: { label: string; k: MetricKey; cur: MetricKey; dir: SortDir; onSort: (k: MetricKey) => void }) {
    const active = cur === k
    return (
        <th onClick={() => onSort(k)} className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider cursor-pointer select-none whitespace-nowrap transition-colors ${active ? 'text-red-500' : 'text-gray-400 hover:text-gray-600'}`}>
...
export default function AnalyticsPage() {
    ...
    useEffect(() => {
        async function loadCampaigns() {
            ...
            if (dbError) { setError('Failed to load campaigns'); return }
    ...
    // Load posts + insights directly from DB
    const loadData = useCallback(async (campaignId: string) => {
        if (!campaignId) return
        setLoading(true)
        setError(null)
        try {
            // Fetch posts with creator name joined
            const { data: postsData, error: postsError } = await supabaseClient
                .from('campaign_posts')
                .select(`
                    id, campaign_id, user_id, platform, media_id,
                    permalink, video_url, thumbnail_url, media_type,
            ...
            const { data: insightsData, error: insightsError } = await supabaseClient
                ...
                .select('id, campaign_id, media_id, likes, comments, views, shares, reach, impressions, saves, last_updated')
                .eq('campaign_id', campaignId)
            ...
            ;(insightsData as Insight[]).forEach((ins) => insightMap.set(ins.media_id, ins))

            //@ts-ignore
            const merged: PostWithInsight[] = (postsData as Post[]).map((p) => ({
                ...p,
    ...
    // Keep drill-down in sync when posts reload
    useEffect(() => {
    ...
    // ── Derived data ──────────────────────────────────────────────────────────
    const tiktokPosts = posts // already filtered to tiktok in query
    const sorted = [...tiktokPosts].sort((a, b) => {
        const av = a.insight?.[sortKey] ?? 0, bv = b.insight?.[sortKey] ?? 0
    ...
    const pieData = [
        ...
        { name: 'Comments', value: comments, color: '#6366f1', pct: engTotal ? Math.round((comments / engTotal) * 100) : 0 },
    ].filter((d) => d.value > 0)

    const creatorPieData = selectedCreator
        ? [
            { name: 'Likes', value: selectedCreator.insight?.likes ?? 0, color: '#f97316' },
            { name: 'Comments', value: selectedCreator.insight?.comments ?? 0, color: '#6366f1' },
            { name: 'Shares', value: selectedCreator.insight?.shares ?? 0, color: '#10b981' },
            { name: 'Saves', value: selectedCreator.insight?.saves ?? 0, color: '#f59e0b' },
        ]
            .filter((d) => d.value > 0)
            .map((d) => {
                const t = (selectedCreator.insight?.likes ?? 0) + (selectedCreator.insight?.comments ?? 0)
                    + (selectedCreator.insight?.shares ?? 0) + (selectedCreator.insight?.saves ?? 0)
    ...
    const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, pct }: any) => {
        ...
        return <text x={x} y={y} fill="white" textAnchor="middle" dominantBaseline="central" fontSize={12} fontWeight={600}>{pct}%</text>
    }
                ...
                <div className="max-w-7xl mx-auto px-6 py-8">
                    ...
                    {!selectedCreator && (
                        <>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                                {([
                                    { label: 'Total Views', value: totals.views, sub: 'All TikTok posts', accent: '#f97316', icon: '👁' },
                                    { label: 'Total Likes', value: totals.likes, sub: `${avgEngRate(tiktokPosts)} avg eng.`, accent: '#6366f1', icon: '❤️' },
                                    { label: 'Comments', value: totals.comments, sub: 'Direct responses', accent: '#10b981', icon: '💬' },
                                ] as const).map((m, i) => (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
                                <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                                    <div className="mb-5">
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">Top performing posts by view count</p>
                                    </div>
                                    {barData.length === 0 ? (
                                        <div className="flex items-center justify-center h-48 text-sm text-gray-300">No data yet</div>
                                    ) : (
                                ...
                                <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                                    ...
                                    {pieData.length === 0 ? (
                                        <div className="flex items-center justify-center h-48 text-sm text-gray-300">No engagement data yet</div>
                                    ) : (
                                        <>
                                            <ResponsiveContainer width="100%" height={160}>
                                                <PieChart>
                                                    <Pie data={pieData} cx="50%" cy="50%" innerRadius={42} outerRadius={72} dataKey="value" labelLine={false} label={renderCustomLabel} strokeWidth={2} stroke="#fff">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mt-2">
                                                {pieData.map((d) => (
                                                    <div key={d.name} className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-xs font-semibold text-gray-700 ml-auto">{d.pct}%</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </>
                            ...
                            {tiktokPosts.some((p) => p.insight) && (
                                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-8">
                                    <div className="mb-5">
                                        <h3 className="text-sm font-semibold text-gray-800">Likes vs Comments vs Shares</h3>
                            ...
                            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-10">
                                <div className="px-6 py-5 border-b border-gray-50 flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">Click "View Analytics" to see a creator's full breakdown</p>
                                    </div>
                                ...
                                {sorted.length === 0 ? (
                                    <div className="py-16 text-center text-sm text-gray-300">No TikTok posts found for this campaign.</div>
                                ) : (
                                        ...
                                        <table className="w-full text-sm">
                                            <thead>
                                                <tr className="bg-gray-50/70">
                                                    ...
                                                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Video</th>
                                                    ...
                                                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Posted</th>
                                                        ...
                                                        <td className="px-4 py-3.5">
                                                            <div className="flex items-center gap-3">
                                                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-400 to-pink-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                                                                    {(p.creator_profiles?.full_name ?? '?')[0].toUpperCase()}
                                                                </div>
                                                                <span className="font-medium text-gray-800 text-sm">{p.creator_profiles?.full_name ?? 'Unknown'}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-3.5">
                                                            <div className="flex items-center gap-2.5">
                                                                ...
                                                                <span className="text-xs font-mono text-gray-400">{p.media_id.slice(-8)}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-3.5 tabular-nums font-semibold text-gray-800">{fmt(p.insight?.views ?? 0)}</td>
                                                        <td className="px-4 py-3.5 tabular-nums text-gray-600">{fmt(p.insight?.likes ?? 0)}</td>
                                                        <td className="px-4 py-3.5 tabular-nums text-gray-600">{fmt(p.insight?.comments ?? 0)}</td>
                                                        <td className="px-4 py-3.5 tabular-nums text-gray-600">{fmt(p.insight?.shares ?? 0)}</td>
                                                        <td className="px-4 py-3.5">
                                                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${engRate(p) === '—' ? 'text-gray-300' : 'bg-red-50 text-red-600'}`}>
                                                                {engRate(p)}
                                                            </span>
                                                        </td>
                                                        <td className="px-4 py-3.5 text-xs text-gray-400">
                                                            {new Date(p.posted_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: '2-digit' })}
                                                        </td>
                                                        <td className="px-4 py-3.5">
                                                            <button onClick={() => setSelectedCreator(p)} className="text-xs font-semibold text-red-500 hover:text-red-700 transition-all whitespace-nowrap">
                                                                View Analytics ↗
                                                            </button>
                                                        </td>
                                                    </tr>
                    ...
                    {selectedCreator && (
                        <div className="anim-card">
                            <button onClick={() => setSelectedCreator(null)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-800 mb-6 transition-colors">
                            ...
                            <div className="flex items-center gap-4 mb-7">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-gray-900">{selectedCreator.creator_profiles?.full_name ?? 'Unknown Creator'}</h2>
                                    <p className="text-sm text-gray-400 mt-0.5">
                                        Posted {new Date(selectedCreator.posted_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                                    </p>
                                </div>
                                <div className="ml-auto"><StatusPill status={selectedCreator.status} /></div>
                            </div>

                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                                {([
                                    { label: 'Views', value: selectedCreator.insight?.views ?? 0, accent: '#f97316', icon: '👁' },
                                    { label: 'Likes', value: selectedCreator.insight?.likes ?? 0, accent: '#6366f1', icon: '❤️' },
                                    { label: 'Comments', value: selectedCreator.insight?.comments ?? 0, accent: '#10b981', icon: '💬' },
                                    { label: 'Shares', value: selectedCreator.insight?.shares ?? 0, accent: '#f59e0b', icon: '🔁' },
                                    { label: 'Reach', value: selectedCreator.insight?.reach ?? 0, accent: '#ec4899', icon: '📡' },
                                    { label: 'Impressions', value: selectedCreator.insight?.impressions ?? 0, accent: '#3b82f6', icon: '📊' },
                                ] as const).map((m, i) => (
                                ...
                                <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                                    <div className="flex items-center justify-between mb-4">
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-400 mt-0.5">Media ID: {selectedCreator.media_id}</p>
                                        </div>
                                        {selectedCreator.permalink && (
                                            <a href={selectedCreator.permalink} target="_blank" rel="noopener noreferrer"
                                    ...
                                    <div className="aspect-video bg-gray-900 rounded-xl overflow-hidden">
                                        {selectedCreator.video_url
                                            ? <video src={selectedCreator.video_url} controls className="w-full h-full object-contain" />
                                            : selectedCreator.permalink
                                                ? <div className="flex items-center justify-center h-full">
                                                    <a href={selectedCreator.permalink} target="_blank" rel="noopener noreferrer" className="text-white text-sm hover:underline">Open on TikTok ↗</a>
                                                  </div>
                                                : <div className="flex items-center justify-center h-full text-gray-500 text-sm">No video available</div>}
                                    </div>
                                </div>
                                ...
                                <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                                    ...
                                    {creatorPieData.length === 0 ? (
                                        ...
                                        <>
                                            <ResponsiveContainer width="100%" height={160}>
                                                <PieChart>
                                                    <Pie data={creatorPieData} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" labelLine={false} label={renderCustomLabel} strokeWidth={2} stroke="#000">
                                                        {creatorPieData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                                                    </Pie>
                                                    <Tooltip content={<CustomPieTooltip />} />
                                                </PieChart>
                                            </ResponsiveContainer>
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm px-6 py-4 flex items-center gap-6">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-red-500 tabular-nums">{engRate(selectedCreator)}</p>
                                    </div>
                                    <div className="w-px h-10 bg-gray-100" />
                                    <div>
                                        <p className="text-xs text-gray-400 font-medium">Post Status</p>
                                        <div className="mt-1"><StatusPill status={selectedCreator.status} /></div>
                                    </div>
                                </div>
                                {selectedCreator.insight?.last_updated && (
                                    <p className="text-xs text-gray-400">Last synced: {new Date(selectedCreator.insight.last_updated).toLocaleString()}</p>
                                )}
                            </div>
                        </div>
                    )}
