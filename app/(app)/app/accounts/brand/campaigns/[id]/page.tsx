...
export default function CampaignDetails() {
        ...
        const fetchCampaign = async () => {
            try {
                ...
                if (error) {
                    ...
                    router.push('/app/accounts/brand/dashboard')
                } else {
    ...
    const calculateCpm = (budget: string | number, views: number) => {
        const budgetNum = parseFloat(String(budget).replace(/[$,]/g, ''))
        if (isNaN(budgetNum) || views <= 0) return 'N/A'
        return `$${((budgetNum / views) * 1000).toFixed(3)}`
    }

    const gotoSubmission = () => {
        router.push(`/app/accounts/brand/campaigns/${id}/submissions/`)
    }
            ...
            <div className="bg-white p-6 rounded-lg space-y-8 border mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 border-b pb-6">
                    ...
                    <div>
                        ...
                        <p className="text-lg text-gray-900 font-bold">{formatViews(campaign.estimated_views)}</p>
                    </div>
                    <div>
            ...
            <div className="bg-white p-6 rounded-lg border mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        {(() => {
                            const referenceImages = getAssetsByCategory('reference_image')
                            const brandAssets = getAssetsByCategory('brand_asset')
                            const brandGuidelines = getAssetsByCategory('brand_guidelines')
                            return (
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div>
                                        <p className="text-sm font-medium text-gray-700 mb-2">Reference Images</p>
                                        {referenceImages.map((asset, i) => (
                                            <Link
                                                key={i}
                                                href={asset.url}
                                                target="_blank"
