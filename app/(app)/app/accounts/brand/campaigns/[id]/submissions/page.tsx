// src/app/app/accounts/brand/campaigns/[id]/submissions/SubmissionsView.tsx
'use client'
...
type SubmissionStatus = 'pending' | 'approved' | 'rejected' | 'draft' | 'admin_reject'
type CampaignStatus = 'inreview' | 'approved' | 'cancelled' | 'expired'
...
const SubmissionsView: React.FC = () => {
    ...
    const closeCampaign = useCallback(
        async (currentStatus: CampaignStatus) => {
            if (currentStatus === 'approved') {
                ...
                if (error) {
                    ...
                    setCampaignMeta((prev) => (prev ? { ...prev, status: 'expired' } : null))
                }
            }
        },
        [campaignId]
    ...
    const fetchSubmissionsAndMeta = useCallback(async () => {
        ...
        const meta = metaData as CampaignMeta
        setCampaignMeta(meta)

        // 2. Fetch Submissions
        const hiddenStatuses: SubmissionStatus[] = ['admin_reject', 'draft']
        ...
        const { data: subsData, error: subsError } = await supabaseClient
            ...
            .not('status', 'in', `(${hiddenStatuses.map((s) => `"${s}"`).join(',')})`)
        ...
        if (subsError) {
            ...
            // 3. Auto-close check: if quota is met and campaign is still approved
            const maxSubs = meta.max_submissions
            const quotaMet = maxSubs !== null && maxSubs > 0 && currentApprovedCount >= maxSubs
            if (quotaMet) {
                await closeCampaign(meta.status)
            }
        }

        setLoading(false)
    }, [campaignId, closeCampaign])
    ...
    const handleNavigateToReview = (submissionId: string) => {
        router.push(`/app/accounts/brand/campaigns/${campaignId}/submissions/${submissionId}/review/`)
    }
    ...
    const submissionsClosed = campaignMeta.status === 'expired' || campaignMeta.status === 'cancelled'
            ...
            >
                <div className="flex items-center">
                    ...
                    <div>
                        ...
                        <p className="text-sm">
                            {submissionsClosed
                                ? `Campaign is ${campaignMeta.status.toUpperCase()}. No further approvals or new submissions are allowed.`
                                : quotaMet
                                ? 'Approval quota met. Approving any new submission requires rejecting an existing one.'
                                : isLimited
                                ? `You can approve ${remainingSlots} more submission(s).`
                ...
                {submissionsClosed && (
                    <span className="font-bold text-red-800 bg-white px-3 py-1 rounded-full shadow-md">
                        {campaignMeta.status.toUpperCase()}
                    </span>
                )}
            </div>
                        ...
                        {submissions.map((submission) => (
                            ...
                            >
                                <div className="flex-grow">
                                    ...
                                    <span
                                        className={`text-xs font-medium px-2 py-0.5 rounded-full mt-1 inline-block ${getStatusStyles(
                                            submission.status
                                        )}`}
                                    >
