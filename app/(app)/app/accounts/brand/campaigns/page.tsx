...
import { fetchBrandProfile, getAuthUser } from '@/lib/supabase/auth/authHelpers'
...
export default function brandCampaignsWorkspace() {
    ...
    /**
     * Used to navigate to particular campaigns
     * @param campaignId
     */
    const handleCampaignClick = (campaignId: string) => {
        router.push(`/app/accounts/brand/campaigns/${campaignId}`)
    }
    ...
    /**Used to handle View Submissions
     *
     * @param e
     * @param campaignId
     */
    ...
    const handleViewSubmissions = (e: React.MouseEvent, campaignId: string) => {
        e.stopPropagation()
        router.push(`/app/accounts/brand/campaigns/${campaignId}/submissions/`)
    }

    /**
     * Used to get the current campaignBrandLogo
     */
    ...
    useEffect(() => {
        const intializeCampaignsWorkspace = async () => {
            try {
                setLoading(true)
                setError(null)
                ...
                /**
                 * Fetch the brandProfileAswell
                 */

                fetchCurrentBrandProfile()

                /**
                 * Get the current campaigns
                ...
                if (error) {
                    toast.error('Error fetching User Campaigns')
                    return
                }

                /**
                 * Set the current Campaigns
                 */
